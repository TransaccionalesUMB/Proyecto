...
-- Actualizar tabla Producto para incluir unidades de medida (si las columnas no existen)
ALTER TABLE producto 
ADD COLUMN IF NOT EXISTS id_unidad_compra INT,
ADD COLUMN IF NOT EXISTS id_unidad_venta INT,
ADD COLUMN IF NOT EXISTS factor_conversion DECIMAL(10,4) DEFAULT 1.0;

-- Intentar agregar las restricciones de clave foránea si no existen
-- Nota: MySQL no tiene una sintaxis directa para "ADD CONSTRAINT IF NOT EXISTS"
-- Por lo tanto, verificamos primero si la restricción existe
...
-- Actualizar tabla Stock para incluir ubicación y lote (si las columnas no existen)
ALTER TABLE stock 
ADD COLUMN IF NOT EXISTS id_ubicacion INT,
ADD COLUMN IF NOT EXISTS id_lote INT;
...
CREATE INDEX IF NOT EXISTS idx_movimiento_producto ON movimiento(id_producto);
CREATE INDEX IF NOT EXISTS idx_movimiento_bodega ON movimiento(id_bodega);
CREATE INDEX IF NOT EXISTS idx_transferencia_producto ON transferencias(id_producto);
...
CREATE INDEX IF NOT EXISTS idx_ubicacion_bodega ON ubicacion_bodega(id_bodega);
...
CREATE INDEX IF NOT EXISTS idx_conteo_inventario ON conteo_inventario(id_inventario);
CREATE INDEX IF NOT EXISTS idx_conteo_producto ON conteo_inventario(id_producto);
CREATE INDEX IF NOT EXISTS idx_precio_especial_producto ON precio_especial(id_producto);
CREATE INDEX IF NOT EXISTS idx_devolucion_factura ON devolucion(id_factura);
CREATE INDEX IF NOT EXISTS idx_item_devolucion_producto ON item_devolucion(id_producto);
